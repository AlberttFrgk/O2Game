...
std::vector<NoteImageType> Key2Type = {
    NoteImageType::LANE_1, NoteImageType::LANE_2, NoteImageType::LANE_3,
    NoteImageType::LANE_4, NoteImageType::LANE_5, NoteImageType::LANE_6,
    NoteImageType::LANE_7,
};

std::vector<NoteImageType> Key2HoldType = {
    NoteImageType::HOLD_LANE_1, NoteImageType::HOLD_LANE_2,
...
std::unordered_map<int, ManiaKeyState> KeyMapping = {
    {0, {Keys::A, false}},     {1, {Keys::S, false}}, {2, {Keys::D, false}},
    {3, {Keys::Space, false}}, {4, {Keys::J, false}}, {5, {Keys::K, false}},
    {6, {Keys::L, false}},
};
...
RhythmEngine::RhythmEngine()
    : m_lanePos{0}, m_laneSize{0}, m_playRectangle{0, 0, 0, 0} {
...
RhythmEngine::~RhythmEngine() { Release(); }
...
bool RhythmEngine::Load(Chart *chart) {
  ...
  for (int i = 0; i < 7; i++) {
    ...
    if (m_eventCallback) {
      m_tracks[i]->ListenEvent([&](GameTrackEvent e) { m_eventCallback(e); });
    }
  ...
  currentX =
      static_cast<int>(std::accumulate(m_laneSize, m_laneSize + 7, 0.0f));
  ...
  if (isSV) {
    m_timings = new VelocityTiming(chart->m_bpms, chart->m_svs,
                                   chart->InitialSvMultiplier);
  } else {
    m_timings = new StaticTiming(chart->m_bpms, chart->m_svs,
                                 chart->InitialSvMultiplier);
  }
  ...
  if (EnvironmentSetup::GetInt("Autoplay") == 1) {
    ...
    std::sort(
        ...
        [](const Autoplay::ReplayHitInfo &a, const Autoplay::ReplayHitInfo &b) {
          if (a.Time == b.Time) {
  ...
  if (audioVolume.size() > 0) {
    try {
      ...
      Logs::Puts("[Gameplay] Invalid volume: %s reverting to 100 value",
                 audioVolume.c_str());
      m_audioVolume = 100;
    }
  }
  ...
  if (audioOffset.size() > 0) {
    try {
      ...
      Logs::Puts("[Gameplay] Invalid offset: %s reverting to 0 value",
                 audioOffset.c_str());
      m_audioOffset = 0;
    }
  }
  ...
  if (autoSound.size() > 0) {
    try {
      ...
      Logs::Puts("[Gameplay] Invalid auto sound: %s reverting to 0 value",
                 autoSound.c_str());
      IsAutoSound = false;
    }
  }
  ...
  if (noteSpeed.size() > 0) {
    try {
      ...
      Logs::Puts("[Gameplay] Invalid notespeed: %s reverting to 210 value",
                 noteSpeed.c_str());
      m_scrollSpeed = 210;
    }
  }
  ...
  if (EnvironmentSetup::GetInt("SongType") == 1) {
    ...
    std::string titleStr =
        std::string(chart->m_title.begin(), chart->m_title.end());
    std::snprintf(buffer, MAX_BUFFER_TXT_SIZE, "Lv.%d %s", chart->m_level,
                  titleStr.c_str());
  } else if (EnvironmentSetup::GetInt("SongType") == 2) {
    m_title = chart->m_title;
    std::string difnameStr =
        std::string(chart->m_difname.begin(), chart->m_difname.end());
    std::string titleStr =
        std::string(chart->m_title.begin(), chart->m_title.end());
    std::snprintf(buffer, MAX_BUFFER_TXT_SIZE, "[%s] %s", difnameStr.c_str(),
                  titleStr.c_str());
  } else {
    m_title = chart->m_title;
    std::string titleStr =
        std::string(chart->m_title.begin(), chart->m_title.end());
    std::snprintf(buffer, MAX_BUFFER_TXT_SIZE, "%d %s", chart->m_level,
                  titleStr.c_str());
  }
  ...
  m_timingLineManager =
      chart->m_customMeasures.size() > 0
          ? new TimingLineManager(this, chart->m_customMeasures)
          : new TimingLineManager(this);
...
bool RhythmEngine::Ready() { return m_state == GameState::NotGame; }

void RhythmEngine::Update(double delta) {
  if (m_state == GameState::NotGame || m_state == GameState::PosGame)
    return;

  // Since I'm coming from Roblox, and I had no idea how to Real-Time sync the
  // audio I decided to use this method again from Roblox project I did in past.
  ...
  for (int i = m_currentSampleIndex; i < m_autoSamples.size(); i++) {
    ...
    if (m_currentAudioPosition >= sample.StartTime) {
      if (sample.StartTime - m_currentAudioPosition < 5) {
        GameAudioSampleCache::Play(sample.Index,
                                   (int)round(sample.Volume * m_audioVolume),
                                   (int)round(sample.Pan * 100));
      }
...
void RhythmEngine::ListenKeyEvent(
    std::function<void(GameTrackEvent)> callback) {
  m_eventCallback = callback;
}

double RhythmEngine::GetAudioPosition() const { return m_currentAudioPosition; }
...
double RhythmEngine::GetTrackPosition() const { return m_currentTrackPosition; }
...
double RhythmEngine::GetNotespeed() const {
  ...
  float value = (float)((speed / 10.0) / (20.0 * m_rate) * scrollingFactor *
                        virtualRatio);
...
double RhythmEngine::GetCurrentBPM() const { return m_currentBPM; }
...
int RhythmEngine::GetBPMAnimationIndex(int maxFrames) const {
  ...
  bool hasSVs = m_currentChart && m_currentChart->m_svs.size() > 0;
  bool isNormalizedSV = hasBPMChanges && hasSVs;
...
double RhythmEngine::GetSongRate() const { return m_rate; }

double RhythmEngine::GetAudioLength() const { return m_audioLength; }

int RhythmEngine::GetGameVolume() const { return m_audioVolume; }

GameState RhythmEngine::GetState() const { return m_state; }

ScoreManager *RhythmEngine::GetScoreManager() const { return m_scoreManager; }
...
int RhythmEngine::GetGuideLineIndex() const { return m_guideLineIndex; }

void RhythmEngine::SetGuideLineIndex(int idx) { m_guideLineIndex = idx; }

void RhythmEngine::UpdateNotes() {
  for (int i = m_currentNoteIndex; i < m_noteDescs.size(); i++) {
    auto &desc = m_noteDescs[i];

    if (m_currentAudioGamePosition + (3000.0 / GetNotespeed()) >
            desc.StartTime ||
        (m_currentTrackPosition - desc.InitialTrackPosition >
         GetPrebufferTiming())) {
...
void RhythmEngine::UpdateGamePosition() {
  ...
  while (m_currentBPMIndex + 1 < m_currentChart->m_bpms.size() &&
         m_currentVisualPosition >=
             m_currentChart->m_bpms[m_currentBPMIndex + 1].StartTime) {
    m_currentBPMIndex += 1;
  }

  while (m_currentSVIndex < m_currentChart->m_svs.size() &&
         m_currentVisualPosition >=
             m_currentChart->m_svs[m_currentSVIndex].StartTime) {
    m_currentSVIndex += 1;
  }

  m_currentTrackPosition = m_timings->GetOffsetAt(
      m_currentVisualPosition,
      m_currentSVIndex); // GetPositionFromOffset(m_currentVisualPosition,
                         // m_currentSVIndex);
...
void RhythmEngine::CreateTimingMarkers() {
  if (m_currentChart->m_svs.size() > 0) {
    ...
    double pos =
        ::round(svs[0].StartTime * m_currentChart->InitialSvMultiplier * 100);
...
const float *RhythmEngine::GetLaneSizes() const { return &m_laneSize[0]; }
...
void RhythmEngine::SetLaneOffset(int offset) { m_laneOffset = offset; }

int RhythmEngine::GetHitPosition() const { return m_hitPosition; }

Vector2 RhythmEngine::GetResolution() const { return m_gameResolution; }

Rect RhythmEngine::GetPlayRectangle() const { return m_playRectangle; }

std::u8string RhythmEngine::GetTitle() const { return m_title; }

TimingBase *RhythmEngine::GetTiming() const { return m_timings; }

JudgeBase *RhythmEngine::GetJudge() const { return m_judge; }
