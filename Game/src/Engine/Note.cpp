...
namespace {
	double CalculateNotePosition(double offset, double initialTrackPos,
								 double hitPosition, double noteSpeed,
								 bool upscroll)
	{
		return hitPosition + ((initialTrackPos - offset) *
							  (upscroll ? noteSpeed : -noteSpeed) / 100);
	}
	...
	const int length_multiplier[4] = { 0, 2, 4, 6 };
} // namespace
...
void Note::Load(NoteInfoDesc *desc)
{
	...
	m_trail_up = NoteImageCacheManager::GetInstance()->DepoolTrail(
		NoteImageType::TRAIL_UP);
	m_trail_down = NoteImageCacheManager::GetInstance()->DepoolTrail(
		NoteImageType::TRAIL_DOWN);
	...
	if (m_head && m_head->m_frames.size() > 0) {
		...
		if (h == 0)
			h = m_head->m_frames[0]->GetOriginalRECT().bottom;

		m_head->Size = UDim2::fromOffset(w, h);
		if (m_tail)
			m_tail->Size = UDim2::fromOffset(w, h);
		if (m_body)
			m_body->Size = UDim2::fromOffset(w, m_body->Size.Y.Offset);
	}
...
void Note::Update(double delta)
{
	...
	if (m_type == NoteType::NORMAL) {
		...
		} else if (m_state == NoteState::HOLD_ON_HOLDING ||
				   m_state == NoteState::HOLD_MISSED_ACTIVE ||
				   m_state == NoteState::HOLD_PASSED) {

			if (m_state == NoteState::HOLD_ON_HOLDING) {
				if (m_lastScoreTime != -1 && audioPos <= m_endTime &&
					audioPos > m_startTime) {
...
void Note::DrawHead(double delta, double headPosY, int guideLineLength,
					Rect &playRect)
{
	...
	if (guideLineLength > 0) {
		...
		m_trail_down->Position =
			m_head->Position + UDim2::fromOffset(m_head->AbsoluteSize.X, 0);
		m_trail_down->AnchorPoint = { 1, 0 };
		m_trail_down->AlphaBlend = true;
		m_trail_down->Draw(delta, &playRect);

		m_trail_up->Position =
			m_head->Position + UDim2::fromOffset(0, -m_head->AbsoluteSize.Y);
		...
		m_trail_up->Position =
			m_head->Position +
			UDim2::fromOffset(m_head->AbsoluteSize.X, -m_head->AbsoluteSize.Y);
...
void Note::DrawBody(double delta, double bodyPosY, double bodyHeight,
					Rect &playRect)
{
...
void Note::DrawTail(double delta, double tailPosY, int guideLineLength,
					Rect &playRect)
{
	...
	if (guideLineLength > 0) {
		...
		m_trail_down->Position =
			m_tail->Position + UDim2::fromOffset(m_tail->AbsoluteSize.X, 0);
		m_trail_down->AnchorPoint = { 1, 0 };
		m_trail_down->AlphaBlend = true;
		m_trail_down->Draw(delta, &playRect);

		m_trail_up->Position =
			m_tail->Position + UDim2::fromOffset(0, -m_tail->AbsoluteSize.Y);
		m_trail_up->Size = UDim2::fromOffset(1, guideLineLength);
		m_trail_up->AnchorPoint = { 0, 1 };
		m_trail_up->AlphaBlend = true;
		m_trail_up->Draw(delta, &playRect);
		...
		m_trail_up->Position =
			m_tail->Position +
			UDim2::fromOffset(m_tail->AbsoluteSize.X, -m_tail->AbsoluteSize.Y);
		m_trail_up->AnchorPoint = { 1, 1 };
		m_trail_up->AlphaBlend = true;
		m_trail_up->Draw(delta, &playRect);
	}
...
void Note::SetTransparency()
{
	...
	if (m_hitResult >= NoteResult::GOOD &&
		m_state == NoteState::HOLD_ON_HOLDING) {
		transparency = 1.0f;
	} else if (m_hitResult >= NoteResult::MISS &&
			   m_state == NoteState::HOLD_MISSED_ACTIVE) {
		transparency = 0.7f;
	}
...
void Note::Render(double delta)
{
	...
	double y1 = CalculateNotePosition(trackPosition, m_initialTrackPosition,
									  1000.0, m_engine->GetNotespeed(), false) /
				1000.0;
	double headPosY =
		lerp(0.0, static_cast<double>(hitPos), static_cast<float>(y1));
	bool isHeadVisible = isWithinRange(headPosY, min, max);

	if (m_type == NoteType::HOLD) {
		double y2 = CalculateNotePosition(trackPosition, m_endTrackPosition, 1000.0,
										  m_engine->GetNotespeed(), false) /
					1000.0;
		double tailPosY =
			lerp(0.0, static_cast<double>(hitPos), static_cast<float>(y2));
		...
		double bodyHeight = std::abs(headPosY - (m_head->AbsoluteSize.Y / 2.0)) -
							(tailPosY - (m_head->AbsoluteSize.Y / 2.0));
...
double Note::GetInitialTrackPosition() const { return m_initialTrackPosition; }

double Note::GetStartTime() const { return m_startTime; }
...
int Note::GetKeysoundId() const { return m_keysoundIndex; }

int Note::GetKeyVolume() const { return m_keyVolume; }

int Note::GetKeyPan() const { return m_keyPan; }

NoteType Note::GetType() const { return m_type; }
...
{
	double overshoot = m_engine->GetGameAudioPosition() - m_startTime;
	if (m_type == NoteType::HOLD) {
		if (m_state == NoteState::HOLD_PRE) {
			m_didHitHead = true;
			m_state = NoteState::HOLD_ON_HOLDING;
			...
			m_hitResult = result;
			m_track->HandleHoldScore(HoldResult::HoldAdd);
			m_track->HandleScore({ result, m_hitPos, false, m_ignore, 2, overshoot });
		} else if (m_state == NoteState::HOLD_MISSED_ACTIVE) {
			m_didHitHead = true;
			m_state = NoteState::HOLD_PASSED;

			m_track->HandleHoldScore(HoldResult::HoldBreak);
			m_track->HandleScore({ result, m_hitPos, true, m_ignore, 2, overshoot });
		}
	} else {
		m_state = NoteState::NORMAL_NOTE_PASSED;
		m_track->HandleScore({ result, m_hitPos, false, m_ignore, 1, overshoot });
	}
}

void Note::OnRelease(NoteResult result)
{
	double overshoot = m_engine->GetGameAudioPosition() - m_endTime;
	if (m_type == NoteType::HOLD) {
		if (m_state == NoteState::HOLD_ON_HOLDING ||
			m_state == NoteState::HOLD_MISSED_ACTIVE) {
			m_lastScoreTime = -1;
...
void Note::SetXPosition(int x) { m_laneOffset = x; }

void Note::SetDrawable(bool drawable) { m_drawAble = drawable; }

void Note::GetNoteSize()
{
	EnvironmentSetup::SetInt("NoteSize",
							 static_cast<int>(m_head->AbsoluteSize.Y));
}
...
bool Note::IsRemoveable() const { return m_state == NoteState::DO_REMOVE; }

bool Note::IsPassed() const
{
	return m_state == NoteState::NORMAL_NOTE_PASSED ||
		   m_state == NoteState::HOLD_PASSED || IsRemoveable();
}

bool Note::IsHeadHit() const { return m_didHitHead; }

bool Note::IsTailHit() const { return m_didHitTail; }
