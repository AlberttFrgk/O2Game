...
void GameplayScene::Render(double delta)
{
	...
	if (m_autoPlay)
		titleStr += (const char8_t *)" [Autoplay]";
...
bool GameplayScene::Attach()
{
	...
	try {
		...
		auto parsePositionsFallback = [&](const std::string &propName, std::vector<PositionValue> &target) {
			...
			if (!propStr.empty()) {
				...
				for (auto &s : splits) {
					...
					if (coords.size() >= 1)
						pv.X = std::stoi(coords[0]);
					if (coords.size() >= 2)
						pv.Y = std::stoi(coords[1]);
					if (coords.size() >= 3)
						pv.AnchorPointX = std::stof(coords[2]);
					if (coords.size() >= 4)
						pv.AnchorPointY = std::stof(coords[3]);
					target.push_back(pv);
				}
			}
		};
		...
		try {
			conKeyLight = manager->GetPosition(SkinGroup::Playing, "KeyLighting");
		} catch (...) {
		}
		...
		try {
			conKeyButton = manager->GetPosition(SkinGroup::Playing, "KeyButton");
		} catch (...) {
		}
		...
		try {
			conHitEffect = manager->GetPosition(SkinGroup::Playing, "HitEffect");
		} catch (...) {
		}
		...
		try {
			conHoldEffect = manager->GetPosition(SkinGroup::Playing, "HoldEffect");
		} catch (...) {
		}
		...
		if (!colStartStr.empty() && !colWidthStr.empty()) {
			...
			if (colWidths.size() >= m_keyCount) {
				...
				for (int i = 0; i < m_keyCount; i++) {
					...
					if (genKeyButton)
						conKeyButton.push_back(btnPos);
					if (genKeyLight)
						conKeyLight.push_back(lgtPos);
					if (genHitEffect)
						conHitEffect.push_back(btnPos);
					if (genHoldEffect)
						conHoldEffect.push_back(btnPos);
		...
		if (!useManiaLayout && conKeyButton.size() < m_keyCount) {
			throw std::runtime_error(
				...
				"positions! (count < " +
				std::to_string(m_keyCount) + ")");
		}

		std::string playfieldImg = manager->GetSkinConfigProp(SkinGroup::Playing, "Key#" + std::to_string(m_keyCount), "Playfield");
		if (playfieldImg.empty())
			playfieldImg = manager->GetImageMapping(SkinGroup::Playing, "Playfield");
		if (playfieldImg.empty())
			playfieldImg = "Playfield";
		...
		if (!imageAlignStr.empty()) {
			...
			for (auto &s : splits) {
				if (s.length() > 0)
					imageAlign.push_back(toupper(s[0]));
				else
					imageAlign.push_back('C');
			}
		} else {
		...
		for (int i = 0; i < m_keyCount; i++) {
			...
			if (btnBase.empty())
				btnBase = manager->GetImageMapping(SkinGroup::Playing, "KeyButton" + std::to_string(i));
			if (btnBase.empty())
				btnBase = "KeyButton" + keyMap[i];

			std::string dwnBase = manager->GetSkinConfigProp(SkinGroup::Playing, "Key#" + std::to_string(m_keyCount), "KeyDown" + std::to_string(i));
			if (dwnBase.empty())
				dwnBase = manager->GetImageMapping(SkinGroup::Playing, "KeyDown" + std::to_string(i));
			if (dwnBase.empty())
				dwnBase = "KeyDown" + keyMap[i];

			std::string lgtBase = manager->GetSkinConfigProp(SkinGroup::Playing, "Key#" + std::to_string(m_keyCount), "KeyLighting" + std::to_string(i));
			if (lgtBase.empty())
				lgtBase = manager->GetImageMapping(SkinGroup::Playing, "KeyLighting" + std::to_string(i));
			if (lgtBase.empty())
				lgtBase = "KeyLighting" + keyMap[i];
			...
			if (useManiaLayout) {
				...
				if (genKeyLight)
					alignSprite(m_keyLighting[i], conKeyLight[i], true); // Always scale KeyLighting
				if (genKeyButton) {
		...
		if (comboNumImg.empty())
			comboNumImg = manager->GetImageMapping(SkinGroup::Playing, "ComboNum");
		if (comboNumImg.empty())
			comboNumImg = "ComboNum";
		...
		if (jamNumImg.empty())
			jamNumImg = manager->GetImageMapping(SkinGroup::Playing, "JamNum");
		if (jamNumImg.empty())
			jamNumImg = "JamNum";
		...
		if (scoreNumImg.empty())
			scoreNumImg = manager->GetImageMapping(SkinGroup::Playing, "ScoreNum");
		if (scoreNumImg.empty())
			scoreNumImg = "ScoreNum";
		...
		if (judgeImg.empty())
			judgeImg = manager->GetImageMapping(SkinGroup::Playing, "Judge");
		if (judgeImg.empty())
			judgeImg = "Judge";
		...
		if (jamGaugeImg.empty())
			jamGaugeImg = manager->GetImageMapping(SkinGroup::Playing, "JamGauge");
		if (jamGaugeImg.empty())
			jamGaugeImg = "JamGauge";
		...
		if (jamLogoImg.empty())
			jamLogoImg = manager->GetImageMapping(SkinGroup::Playing, "JamLogo");
		if (jamLogoImg.empty())
			jamLogoImg = "JamLogo";
		...
		if (lifeBarImg.empty())
			lifeBarImg = manager->GetImageMapping(SkinGroup::Playing, "LifeBar");
		if (lifeBarImg.empty())
			lifeBarImg = "LifeBar";
		...
		if (longNoteNumImg.empty())
			longNoteNumImg = manager->GetImageMapping(SkinGroup::Playing, "LongNoteNum");
		if (longNoteNumImg.empty())
			longNoteNumImg = "LongNoteNum";
		...
		if (statsNumImg.empty())
			statsNumImg = manager->GetImageMapping(SkinGroup::Playing, "StatsNum");
		if (statsNumImg.empty())
			statsNumImg = "StatsNum";
		...
		if (exitImg.empty())
			exitImg = manager->GetImageMapping(SkinGroup::Playing, "Exit");
		if (exitImg.empty())
			exitImg = "Exit";
		...
		if (lnLogoImg.empty())
			lnLogoImg = manager->GetImageMapping(SkinGroup::Playing, "LongNoteLogo");
		if (lnLogoImg.empty())
			lnLogoImg = "LongNoteLogo";
		...
		if (comboLogoImg.empty())
			comboLogoImg = manager->GetImageMapping(SkinGroup::Playing, "ComboLogo");
		if (comboLogoImg.empty())
			comboLogoImg = "ComboLogo";
		...
		if (playTimeNumImg.empty())
			playTimeNumImg = manager->GetImageMapping(SkinGroup::Playing, "PlayTimeNum");
		if (playTimeNumImg.empty())
			playTimeNumImg = "PlayTimeNum";
		...
		if (targetBarImg.empty())
			targetBarImg = manager->GetImageMapping(SkinGroup::Playing, "TargetBar");
		if (targetBarImg.empty())
			targetBarImg = "TargetBar";
		...
		if (playfooterImg.empty())
			playfooterImg = manager->GetImageMapping(SkinGroup::Playing, "Playfooter");
		if (playfooterImg.empty())
			playfooterImg = "PlayfieldFooter";
		...
		if (useManiaLayout)
			m_game->SetColumnWidths(colWidths);
		if (!noteHeightStr.empty())
			m_game->SetNoteHeight(std::stoi(noteHeightStr));
		...
		auto OnTrackEvent = [&](GameTrackEvent e) {
			if (e.IsKeyEvent) {
				...
				if (e.IsHitEvent) {
					...
					if (e.IsHitLongEvent) {
						m_holdEffect[e.Lane]->Reset(e.Overshoot / 1000.0);
						m_holdEffect[e.Lane]->FlipX = flip;
						m_drawHit[e.Lane] = false;
						m_drawHold[e.Lane] = e.State;

						if (!e.State) {
							m_hitEffect[e.Lane]->Reset(e.Overshoot / 1000.0);
		...
		if (hitEffectImg.empty())
			hitEffectImg = manager->GetImageMapping(SkinGroup::Playing, "HitEffect");
		if (hitEffectImg.empty())
			hitEffectImg = "HitEffect";

		std::string holdEffectImg = manager->GetSkinConfigProp(SkinGroup::Playing, "Key#" + std::to_string(m_keyCount), "HoldEffect");
		if (holdEffectImg.empty())
			holdEffectImg = manager->GetImageMapping(SkinGroup::Playing, "HoldEffect");
		if (holdEffectImg.empty())
			holdEffectImg = "HoldEffect";
