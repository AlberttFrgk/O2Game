...
void SceneManager::Update(double delta) {
  if (m_nextScene != nullptr) {
    ...
    if (!m_nextScene->Attach()) {
      MsgBox::ShowOut("EstEngine Error", "Failed to init next scene",
                      MsgBoxType::OK, MsgBoxFlags::BTN_ERROR);
      m_parent->Stop();
      return;
    }
    ...
    if (m_currentScene) {
      ...
      if (!curScene->Detach()) {
        MsgBox::ShowOut("EstEngine Error", "Failed to detach current screen",
                        MsgBoxType::OK, MsgBoxFlags::BTN_ERROR);
        m_parent->Stop();
        return;
      }
  ...
  if (m_nextOverlay != nullptr) {
    ...
    if (!m_nextOverlay->Attach()) {
      MsgBox::ShowOut("EstEngine Error", "Failed to init next overlay",
                      MsgBoxType::OK, MsgBoxFlags::BTN_ERROR);
      m_parent->Stop();
      return;
    }
    ...
    if (m_currentOverlay) {
      ...
      if (!curOverlay->Detach()) {
        MsgBox::ShowOut("EstEngine Error", "Failed to detach current overlay",
                        MsgBoxType::OK, MsgBoxFlags::BTN_ERROR);
        m_parent->Stop();
        return;
      }
  ...
  if (m_nextScene != nullptr) {
    ...
    if (m_currentScene) {
      if (!m_currentScene->Detach()) {
        MsgBox::ShowOut("EstEngine Error", "Failed to detach current scene",
                        MsgBoxType::OK, MsgBoxFlags::BTN_ERROR);
        m_parent->Stop();
        return;
      }
...
void SceneManager::ChangeScene(int idx) {
  ...
  std::thread([idx]() { s_instance->IChangeScene(idx); }).detach();
}
...
void SceneManager::OverlayShow(int idx) {
  ...
  if (map.find(idx) == map.end()) {
    ...
    MsgBox::ShowOut("EstEngine Error", msg.c_str(), MsgBoxType::OK,
                    MsgBoxFlags::BTN_ERROR);
    return;
  }
...
void SceneManager::SetParent(Game *parent) { m_parent = parent; }
...
int SceneManager::GetCurrentSceneIndex() const { return m_currentSceneId; }

int SceneManager::GetLastSceneIndex() const { return m_lastSceneId; }

void SceneManager::DisplayFade(int transparency,
  ...
  std::thread([&, transparency, callback] {
    ...
    while (static_cast<int>(s_instance->m_parent->GetDisplayFade()) !=
           transparency) {
...
void SceneManager::ExecuteAfter(int ms_time, std::function<void()> callback) {
  ...
  info.time =
      std::chrono::system_clock::now() + std::chrono::milliseconds(ms_time);
...
void SceneManager::StopGame() { m_parent->Stop(); }
