...
{
	if (m_textures.empty())
		return; // Safety check to ensure m_textures is not empty
...
void Sprite2D::Reset(double overshoot)
{
	m_currentIndex = 0;
	m_currentTime = static_cast<float>(overshoot);
}

Texture2D *Sprite2D::GetTexture()
{
	if (m_textures.empty())
		return nullptr; // Safety check to ensure m_textures is not empty
