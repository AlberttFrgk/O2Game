...
bool Renderer::Create(RendererMode mode, GameWindow *window, bool failed) {
  ...
  try {
    ...
    if (bUsedSDLRenderer) {
      ...
      if (failed) {
        Logs::Puts("[Renderer] Failed to create renderer with backend: %s, and "
                   "fallback to %s",
                   rendererName.c_str(), SDL_GetCurrentVideoDriver());
      }
      ...
      try {
        ...
        MsgBox::ShowOut("EstEngine Error",
                        "Failed to load vulkan functions, fallback to OpenGL",
    ...
    MsgBox::ShowOut("EstEngine Error", e.what(), MsgBoxType::OK,
...
SDL_BlendMode Renderer::GetSDLBlendMode() { return m_blendMode; }

VulkanEngine *Renderer::GetVulkanEngine() { return m_vulkan; }
...
bool Renderer::IsVulkan() { return GetVulkanEngine() != nullptr; }
